...
class BitrixReportDataProviderTests(TestCase):
    ...
    def test_telephony_long_range_is_split_into_short_bitrix_windows(self):
        from datetime import datetime
        from zoneinfo import ZoneInfo

        from apps.reports.services.calculators.telephony_calculator import (
            CALL_QUERY_WINDOW_DAYS,
            iter_call_query_windows,
            load_call_rows,
        )

        moscow = ZoneInfo("Europe/Moscow")
        date_from = datetime(2026, 8, 3, 0, 0, tzinfo=moscow)
        date_to = datetime(2026, 9, 6, 23, 59, 59, tzinfo=moscow)
        windows = iter_call_query_windows(date_from, date_to)

        self.assertGreater(len(windows), 1)
        self.assertEqual(windows[0][0], date_from)
        self.assertEqual(windows[-1][1], date_to)
        self.assertLessEqual((windows[0][1] - windows[0][0]).days, CALL_QUERY_WINDOW_DAYS)

        recorded = []

        class TrackingCallsClient:
            def call_list(self, method, params=None, *, max_pages=None):
                recorded.append((params or {}).get("FILTER"))
                return []

        load_call_rows(
            client=TrackingCallsClient(),
            date_from=date_from,
            date_to=date_to,
            bitrix_datetime=lambda value: value.isoformat(),
        )

        self.assertEqual(len(recorded), len(windows))
        self.assertTrue(recorded[0][">=CALL_START_DATE"].startswith("2026-08-03"))
...
class CrmWarehouseTests(TestCase):
    ...
    def test_oauth_reauth_skips_warehouse_sync(self):
        from apps.reports.services.crm_warehouse_sync import sync_portal_crm_warehouse

        self._grant_pro()
        self.portal.oauth_reauth_required = True
        self.portal.save(update_fields=["oauth_reauth_required", "updated_at"])
    ...
    def test_ready_warehouse_does_not_cover_telephony_without_source_coverage(self):
        ...
        date_from = now - timedelta(days=34)
        self.assertFalse(
    ...
    def test_auto_report_keeps_covered_sources_in_mysql_if_telephony_is_uncovered(self):
        from datetime import timedelta
        from unittest.mock import patch
        ...
        with patch.object(
            BitrixReportDataProvider,
            "_load_deals",
            side_effect=AssertionError("Covered deals must stay in the warehouse"),
        ), patch.object(
            BitrixReportDataProvider,
            "_load_calls",
            return_value=[],
            ...
            result = provider.build_preview(
                filters={
                    ...
                    "selectedMetricIds": ["deals_created", "calls_total"],
                    "metricMode": "count",
                    "chartDisplayMode": "sum",
                },
                context=ReportDataProviderContext(
    ...
    def test_report_rests_only_the_gap_before_recent_warehouse_days(self):
        ...
        def fake_load_deals(self, **kwargs):
            deal_windows.append((kwargs["date_from"], kwargs["date_to"]))
            return [
                {
                    "ID": "2",
                    "TITLE": "August deal",
                    "DATE_CREATE": (now - timedelta(days=34)).isoformat(),
                    "STAGE_ID": "C0:NEW",
                    "OPPORTUNITY": "50",
                }
            ]
        ...
        provider = BitrixReportDataProvider(rest_client_factory=FakeBitrixRestClient)
        with patch.object(BitrixReportDataProvider, "_load_deals", fake_load_deals):
            result = provider.build_preview(
                filters={
                    "period": "days",
                    "dateRange": {
                        "from": (timezone.localtime(now) - timedelta(days=34)).date().isoformat(),
    ...
    def test_repair_does_not_jump_backfill_to_ancient_modified_row(self):
        ...
        upsert_source_rows(
            ...
            rows=[
                {
                    "ID": "old",
                    "TITLE": "Modified old deal",
                    "DATE_CREATE": ancient.isoformat(),
                    "STAGE_ID": "C0:NEW",
                    "OPPORTUNITY": "10",
                },
                {
        ...
        with patch(
            ...
            "apps.reports.services.crm_warehouse_sync.WAREHOUSE_BACKFILL_CHUNKS_PER_RUN",
            1,
        ):
            result = sync_portal_crm_warehouse(self.portal.id)
        ...
        self.assertLessEqual(max(item["date_to"] for item in calls), recent + timedelta(days=1))
        self.assertGreater(max(item["date_to"] for item in calls), now - timedelta(days=40))

    def test_incremental_does_not_mark_180_day_source_coverage(self):
        from datetime import timedelta
        ...
        for source_id, raw_ranges in coverage.items():
            for item in raw_ranges:
                start = parse_datetime(item["from"])
                self.assertIsNotNone(start)
                self.assertGreater(start, now - timedelta(days=20), source_id)
    ...
    def test_running_sync_still_serves_ready_warehouse_and_status(self):
        ...
        from apps.reports.models import PortalCrmRow, PortalCrmSyncState
        from apps.reports.services.crm_warehouse import serialize_fast_reports, upsert_source_rows

        self._grant_pro()
        now = timezone.now()
        PortalCrmSyncState.objects.create(
            portal=self.portal,
            status=PortalCrmSyncState.Status.RUNNING,
            coverage_from=now - timedelta(days=180),
            coverage_to=now - timedelta(minutes=1),
            next_chunk_to=now - timedelta(days=180),
            progress_percent=100,
        ...
        with patch.object(
            ...
            result = provider.build_preview(
                filters={
                    "period": "days",
                    "dateRange": {"from": "2026-05-01", "to": "2026-09-07"},
                    "selectedSources": ["deal-default", "lead-default"],
                    "selectedMetricIds": ["deals_created", "leads_created"],
                    "metricMode": "money",
                    "chartDisplayMode": "sum",
    ...
    def test_progress_100_with_short_coverage_resumes_backfill(self):
        from datetime import timedelta
        ...
        self.assertEqual(result["mode"], "backfill")
        self.assertTrue(calls)
        self.assertLessEqual(max(item["date_to"] for item in calls), oldest + timedelta(days=1))

        ...
        {billingHasPro && fastReportsBannerVisible && fastReportsStatus ? (
          <div className={`report-status-bar ${fastReportsStatus === 'ready' ? 'is-ready' : 'is-info'}`}>
            {fastReportsStatus === 'ready' ? (
              <span>
                Данные за последние 6 месяцев загружены. Отчёты за этот период и уже сохранённые
                считаются быстрее.
              </span>
            ) : (
              <span>
                Загружаем данные за последние 6 месяцев, чтобы отчёты считались быстрее.
                Приложением можно пользоваться: сейчас отчёт строится из Битрикс24.
              </span>
            )}
          </div>
        ) : null}

...
const buildPeriodPoints = (period: Period, range: DateRange) => {
  const { start, end } = normalizeDateRange(range);
...
const metricValue = (metric: MetricRow, index: number, period: Period) => {
  const periodFactor = period === 'hours' ? 0.05 : period === 'days' ? 0.9 : period === 'weeks' ? 5.2 : 18;
  const trendStep = period === 'hours' ? 0.0008 : period === 'days' ? 0.012 : period === 'weeks' ? 0.025 : 0.035;
...
export const buildReportData = (period: Period, range: DateRange = defaultDateRange): ReportPoint[] =>

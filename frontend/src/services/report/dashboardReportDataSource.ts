import {
  loadDashboardPortalEmployees,
  loadDashboardReportCatalog,
  loadDashboardReportPreview,
} from '../api/dashboardReportApiClient';
...
export const dashboardReportDataSource: ReportDataSource = {
  async loadCrmSources() {
    const catalog = await loadDashboardReportCatalog();
  ...
  async loadReportData(filters) {
    const preview = await this.loadReportPreview(filters);

    return preview.data;
  },

  async loadReportPreview(filters) {
    const preview = await loadDashboardReportPreview(filters);
  ...
  async loadPortalEmployees(): Promise<PortalEmployeeItem[]> {
    return loadDashboardPortalEmployees();
  },
  ...
  getInitialReportData(_filters): ReportPoint[] {
    return [];
  },
};
